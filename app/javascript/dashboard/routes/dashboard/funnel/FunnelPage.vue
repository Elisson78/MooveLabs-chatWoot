...
const getLabelStyle = labelTitle => {
  const label = accountLabels.value.find(l => l.title === labelTitle);
  if (label) {
    return {
      backgroundColor: label.color,
      color: getCorrectTextColor(label.color),
    };
  }
  return {};
};
...
const getCardBorderStyle = conversation => {
  const color = getCardColor(conversation);
